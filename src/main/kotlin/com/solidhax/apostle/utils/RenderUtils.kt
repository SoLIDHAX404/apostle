...
import com.solidhax.apostle.utils.render.CustomRenderType
...
object RenderBatchManager {
    ...
    init {
        @EventHandler
        fun onRenderEventLast(event: RenderEvent.Last) {
            val matrix = event.context.matrices()
...
fun RenderEvent.Extract.drawTexturedQuad(
    ...
    consumer.texturedQuads.add(TexturedQuadData(texture, bl, tl, tr, br, -rz, 0f, rx, color.toMinecraftColor(), depth))
}
...
fun RenderEvent.Extract.drawLine(points: Collection<Vec3>, color1: Color, color2: Color, depth: Boolean, thickness: Float = 3f) {
    ...
    val rgba1 = color1.toMinecraftColor()
    val rgba2 = color2.toMinecraftColor()
...
fun RenderEvent.Extract.drawWireFrameBox(aabb: AABB, color: Color, thickness: Float = 3f, depth: Boolean = false) {
    consumer.wireBoxes.add(
        BoxData(aabb, color.red.toFloat() / 255f, color.green.toFloat() / 255f, color.blue.toFloat() / 255f, color.alpha.toFloat() / 255f, thickness, depth)
    )
}
...
fun RenderEvent.Extract.drawCylinder(
    ...
    val rgba = color.toMinecraftColor()
